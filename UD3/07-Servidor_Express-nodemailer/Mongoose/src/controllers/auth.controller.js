...
export const register = async (req, res) => {
    try {
        ...
        const activacionToken = generarAccessToken({ email });
        ...
        const nuevoUsuario = await Usuario.create({
            ...
            activacionToken,
            activacionTokenExpira: Date.now() + (24 * 60 * 60 * 1000)
        });

        //enviar email de activación
        const html = contenidoHTML(email, name, activacionToken);
        await enviarEmailActivacion(email, "Activa tu cuenta", html);
...
const contenidoHTML = (name, tokenActivacion) => {
    const urlActivacion = `${URL}:${PORT}/api/activa-email/${tokenActivacion}`
    return `
            <!DOCTYPE html>
            <html>
            <head>
...
export const activaCuenta = async (req, res) => {
    try {
        ...
        const usuarioExiste = await Usuario.findOne({activacionToken:token});
        if(!usuarioExiste){
            return res.status(403).json({
                message: "Token de activación inválido"
            })
        }
        //Comprobar que la cuenta está inactiva.

        if(usuarioExiste.active){
        ...
        if(usuarioExiste.activacionTokenExpira < Date.now()){
            return res.status(405).json({
        ...
        usuarioExiste.active = true;
        usuarioExiste.activacionToken = undefined;
        usuarioExiste.activacionTokenExpira = undefined;
